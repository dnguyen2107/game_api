...
chestApi.getChestById = function(req,res,next) {
    ...
    Chest.findOne({_id: chestId}, function(err, chest) {
        if(err) {
            return res.status(200).send({ 
                status: 505, 
...
chestApi.deleteChestById = function(req,res,next) {
	...
	Chest.findOne({_id: chestId}, function(err, chest) {
        ...
        if(((userId === chest.sender.toString()) && (chest.send_date.getTime() > Date.now()))
        	|| ((chest.send_date.getTime() < Date.now()) && (_.indexOf(chest.recipients, userId) >= 0))) {
        	Chest.remove({_id: req.params.id}, function(err, chest) {
		        if(err)
		            return res.send(err);
		        return res.status(200).send({
		            status: 200,
...
chestApi.updateChestById = function(req,res,next) {
    ...
    Chest.findOne({_id: chestId}, function(err, chest) {
        ...
        if(userId === chest.sender.toString() && chest.send_date.getTime() > Date.now()) {
        	Chest.update({_id: req.params.id}, { $set: req.body}, {upsert:false, runValidators:true}, function (err) {
	            if (err) {
	                return res.status(200).send({
	                    status: 500,
	                    message: 'Internal Server Error'
...
chestApi.getChestsForUser = function(req, res, next) {
	...
	Chest.find({sender: mongoose.Types.ObjectId(userId)}, function(err, sentChests) {
		if(err) {
            return res.status(200).send({ 
                status: 505, 
        ...
        Chest.find({recipients: userId}, function(err, receivedChests) {
        	if(err) {
	            return res.status(200).send({ 
	                status: 505, 
	                message: "Internal Server Error" + err
...
chestApi.sendChest = function(req, res, next) {
	...
	//retrieve Chest
	Chest.find({_id : chestId}, function(err, chest) {
		if(err) {
            return res.status(200).send({ 
                status: 505, 
                message: "Internal Server Error" + err
        ...
        if(recipients) {
        	Chest.update({_id: chestId}, { $set: {send_date: sendDate, recipients: recipients}}, {upsert:false, runValidators:true}, function (err) {
        		 if (err) {
	                return res.status(200).send({
	                    status: 500,
	                    message: 'Internal Server Error'
        	...
        	Chest.update({_id: chestId}, { $set: {send_date: sendDate}}, {upsert:false, runValidators:true}, function (err) {
        		 if (err) {
	                return res.status(200).send({
	                    status: 500,
	                    message: 'Internal Server Error'
...
chestApi.addContent = function(req, res, next) {
	...
	Chest.findOne({_id: chestId}, function(err, chest) {
		...
		var newEl = {content: req.body.content_id, information: req.body.information};
